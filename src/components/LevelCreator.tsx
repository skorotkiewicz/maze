...
import { Download, Upload, Play, ArrowLeft, Eraser, Square, Circle, MousePointer2 } from 'lucide-react';
...
export function LevelCreator({ onBack, onPlay }: LevelCreatorProps) {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (tool === 'wall') {
      ...
      setLevel(prev => ({ ...prev, end: { ...prev.end, x: pos.x - prev.end.width/2, y: pos.y - prev.end.height/2 } }));
    } else if (tool === 'eraser') {
  ...
  const handleExport = () => {
    ...
    downloadAnchorNode.setAttribute("download", `${level.name.replace(/\s+/g, '-').toLowerCase()}.json`);
    ...
    <div className="min-h-screen bg-zinc-900 text-zinc-100 p-8 flex gap-8">
      ...
      <div className="w-64 flex flex-col gap-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-xs text-zinc-500 uppercase tracking-wider font-medium">Level Name</label>
            <input 
              type="text" 
              value={level.name}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs text-zinc-500 uppercase tracking-wider font-medium">Width</label>
              <input 
                type="text" 
                value={level.size.width}
                onChange={(e) => setLevel(prev => ({ ...prev, size: { ...prev.size, width: Math.max(100, Number(e.target.value)) } }))}
                className="w-full bg-zinc-800 border border-zinc-700 rounded px-3 py-2 mt-1 focus:outline-none focus:border-indigo-500 transition-colors"
              />
            </div>
            <div>
              <label className="text-xs text-zinc-500 uppercase tracking-wider font-medium">Height</label>
              <input 
                type="text" 
                value={level.size.height}
                onChange={(e) => setLevel(prev => ({ ...prev, size: { ...prev.size, height: Math.max(100, Number(e.target.value)) } }))}
                className="w-full bg-zinc-800 border border-zinc-700 rounded px-3 py-2 mt-1 focus:outline-none focus:border-indigo-500 transition-colors"
              />
            </div>
          </div>
          ...
          <div>
            <label className="text-xs text-zinc-500 uppercase tracking-wider font-medium mb-2 block">Tools</label>
            <div className="grid grid-cols-2 gap-2">
              <ToolButton 
                active={tool === 'wall'} 
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        active ? "bg-indigo-600 text-white shadow-lg" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200"
      )}
    >
      {icon}
